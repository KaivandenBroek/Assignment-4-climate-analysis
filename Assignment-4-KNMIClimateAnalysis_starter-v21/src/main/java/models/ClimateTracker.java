...
public class ClimateTracker {
    ...
    /**
     * calculates for each station how many Measurement instances have been
     * registered
     * 
     * @return
     */
    public Map<Station, Integer> numberOfMeasurementsByStation() {
        // TODO build a map resolving for each station its number of registered
    ...
    /**
     * calculates for each station how many valid values it has available for the
     * measurement quantity
     * that can be accessed by the mapper.
     * invalid values are registered as Double.NaN. They originate from empty or
     * corrupt data in the source file
     * 
     * @param mapper a getter method that accesses the selected quantity from a
    ...
    /**
     * Calculates for each calendar year in the dataset the average daily
     * temperatures
     * across all days in the year and all stations in this tracker
     * (invalid values shall be excluded from the averaging)
     * 
     * @return a map(Y,T) that provides for each year Y the average temperature T of
     *         that year
     */
    ...
    /**
     * Calculates for each calendar year in the dataset the maximum of the selected
     * daily quantity
     * across all days in the year and all stations in this tracker
     * (invalid values shall be excluded from the maximum aggregation)
     * (this method can be reused for maximum aggregation of different quantities in
     * the Measurement data
     * 
     * @param mapper a getter function on the Measurement class
     *               that selects the appropriate value for the maximum aggregation
     *               procedure
     * @return a map(Y,Q) that provides for each year Y the maximum value Q of the
     *         specified quantity
     */
    ...
    /**
     * Calculates for each of the 12 calendar months the average daily hours of
     * sunshine
     * across all years and all stations
     * The graph of these numbers can be found in touristic brochures.
     * (invalid values shall be excluded from the averaging)
     * 
     * @return a map(M,SQ) that provides for each month M the average daily sunshine
     *         hours SQ across all times
     */
    public Map<Month, Double> allTimeAverageDailySolarByMonth() {
        // TODO build a map collecting for each month the average value of daily
        // sunshine hours
     ...
     * The coldest year is defined as the year with the lowest total sum of daily
     * minimum temperatures below zero
     * accumulated across all stations
     * I.e. any daily value of a minimum temperature at a station that is above zero
     * should not be included in its sum for the year
     * The lowest yearsum of negative minimum temperatures indicates the coldest
     * year.
     * 
    ...
    public int coldestYear() {
        ...
        // hint: first build a helper map that accumulates the yearsums of negative
    ...
    /**
     * traverses the purchases vault recursively and processes every data file that
     * it finds
     * 
     * @param filePath
     */
    public void importMeasurementsFromVault(String filePath) {
        ...
        if (file.isDirectory()) {
            ...
            // merge all purchases of all files and sub folders from the filesInDirectory
            // list, recursively.
            for (File f : filesInDirectory) {
                this.importMeasurementsFromVault(f.getAbsolutePath());
            }
        } else if (file.getName().matches(MEASUREMENTS_FILE_PATTERN)) {
            // the file is a regular file that matches the target pattern for raw purchase
            // files
    ...
    private static <K, E> void importItemsFromFile(Map<K, E> items, String filePath, String headerPrefix,
        ...
        // originalNumItems, filePath);
    }

    /**
     * imports another batch of raw purchase data from the filePath text file
     * and merges the purchase amounts with the earlier imported and accumulated
     * collection in this.purchases
     * 
     * @param filePath
     */
    private void importMeasurementsFromFile(String filePath) {
        ...
        // import all measurements from the specified file into the
        // newMeasurementsByDate map
